...
import standalone.com.sun.tools.javac.comp.Annotate.AnnotationTypeMetadata;
...
import static standalone.com.sun.tools.javac.code.Flags.ANNOTATION;
import static standalone.com.sun.tools.javac.code.Flags.SYNCHRONIZED;
import static standalone.com.sun.tools.javac.code.Kinds.*;
import static standalone.com.sun.tools.javac.code.Kinds.Kind.*;
import static standalone.com.sun.tools.javac.code.Scope.LookupKind.NON_RECURSIVE;
import static standalone.com.sun.tools.javac.code.Scope.LookupKind.RECURSIVE;
import static standalone.com.sun.tools.javac.code.TypeTag.*;
import static standalone.com.sun.tools.javac.code.TypeTag.WILDCARD;

import static standalone.com.sun.tools.javac.tree.JCTree.Tag.*;
 ...
 **************************************************************************/
     ...
     */
    <T> void compareAndRemove(java.util.List<T> list, standalone.java.util.function.ToIntBiFunction<? super T, ? super T> comparer) {
        ...
        if (isRecordField) {
            ...
            if (!appliesToRecords) {
                ...
                if (metadata == null || metadata.isEmpty()) {
                     ...
                     * whether there are SE5 annotations on a VarDef viz standalone.com.sun.tools.javac.code.TypeAnnotations.TypeAnnotationPositions.visitVarDef
                     */
                }
            }
        }
 ...
 **************************************************************************/
    ...
    void checkSwitchCaseStructure(List<JCCase> cases) {
        for (List<JCCase> l = cases; l.nonEmpty(); l = l.tail) {
            JCCase c = l.head;
            if (c.labels.head instanceof JCConstantCaseLabel) {
              JCConstantCaseLabel constLabel = (JCConstantCaseLabel)c.labels.head;
                if (TreeInfo.isNull(constLabel.expr)) {
                    if (c.labels.tail.nonEmpty()) {
                        if (c.labels.tail.head instanceof JCDefaultCaseLabel) {
        ...
        if (isCaseStatementGroup) {
            ...
            for (List<JCCase> l = cases; l.nonEmpty(); l = l.tail) {
                ...
                if (previousCompletessNormally &&
                    c.stats.nonEmpty() &&
                    c.labels.head instanceof JCPatternCaseLabel &&
                    hasBindings(((JCPatternCaseLabel)c.labels.head).pat)) {
    ...
    void checkSwitchCaseLabelDominated(List<JCCase> cases) {
                    ...
                    if (types.isSubtype(currentType, testType) &&
                        ...
                        if (label instanceof JCConstantCaseLabel) {
                            dominated |= !(testCaseLabel instanceof JCConstantCaseLabel) &&
                                   ...
                                   testCaseLabel instanceof JCPatternCaseLabel &&
                                   TreeInfo.unguardedCase(testCase)) {
                            dominated = patternDominated(((JCPatternCaseLabel)testCaseLabel).pat,
                                                         ((JCPatternCaseLabel)label).pat);
                        }
                        if (dominated) {
                            log.error(label.pos(), Errors.PatternDominated);
                        }
        ...
        private boolean patternDominated(JCPattern existingPattern, JCPattern currentPattern) {
            ...
            } else if (currentPattern instanceof JCRecordPattern) {
              JCRecordPattern currentRecordPattern = (JCRecordPattern)currentPattern;
                if (existingPattern instanceof JCBindingPattern ||
                    existingPattern instanceof JCAnyPattern) {
                    return true;
                } else if (existingPattern instanceof JCRecordPattern) {
                    List<JCPattern> existingNested = ((JCRecordPattern)existingPattern).nested;
